      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
        {FACTS.map(({ n, l, img }, index) => (
          <div key={l} className="group relative">
            {/* Amazed Cat overlay peeking out outside the card boundary */}
            {index === 2 && (
              <img
                src="/hack36/cat-amaze.png"
                alt="Amazed Cat"
                className="absolute bottom-0 -top-24 left-1/2 -translate-x-0.5 w-24 h-24 object-contain z-30 pointer-events-none"
              />
            )}

            <div className="relative bg-zinc-950 rounded-[2.5rem] overflow-hidden min-h-[320px]  flex flex-col justify-end p-8 md:p-10 border border-zinc-900 transition-all hover:border-zinc-800">
